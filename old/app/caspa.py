...
#		self.filemenu.add_command(label="New protocol", command=self.parent.protocol.newProtocol)
		...
		self.filemenu.config(bg=self.parent.headcolor, fg=self.parent.headfc, bd=0, activebackground=self.parent.buttoncolor)
		self.helpmenu.config(bg=self.parent.headcolor, fg=self.parent.headfc, bd=0, activebackground=self.parent.buttoncolor)
...
class Log(tk.Frame):
	...
	def renderLog(self):
		self.log = tk.Text(self.logframe.bodyframe, height="15", width="120", bg=self.parent.lightercolor, bd=0, fg=self.parent.bodyfc)
...
#		self.lengths = []
		...
		for i in range(1, 10):
			...
			tk.Radiobutton(rframe, text="PEEK", variable=tubingtype, value=1, indicator=0, offrelief='flat', bg=self.parent.lightercolor, highlightthickness=0).grid(row=i, column=0)
			#tk.Label(rframe, text="PEEK", bg=self.parent.lightercolor, fg=self.parent.bodyfc).grid(row=i, column=1)
			tk.Radiobutton(rframe, text="non-PEEK", variable=tubingtype, value=2, indicator=0, offrelief='flat', bg=self.parent.lightercolor, fg=self.parent.bodyfc, cursor='hand2', selectcolor=self.parent.buttoncolor, highlightthickness=0).grid(row=i, column=1)
...
#			self.lengths.append(length)
		...
		tk.Button(pf, text="Prime Ports", command=lambda: self.parent.protocol.primePorts(self.portstoprime, self.volume, self.tubingtypes), relief='flat', bg=self.parent.buttoncolor, activebackground=self.parent.wrapcolor, fg=self.parent.buttonfc).pack(padx=5, side='left')

		#ttk.Separator(pframe, orient='horizontal').grid(row=len(self.portstoprime)+3, column=0, columnspan=3, sticky='ew')

		tk.Button(pframe, text="Return Port Contents", command=lambda: self.parent.protocol.returnPortContents(self.portstoprime), relief='flat', bg=self.parent.buttoncolor, activebackground=self.parent.wrapcolor, fg=self.parent.buttonfc).grid(row=len(self.portstoprime)+4, column=0, columnspan=2, padx=5, pady=5)
		...
		tk.Button(cf, text="Calibrate", command=lambda: self.parent.protocol.calibrateOutput(self.calibvolume), relief='flat', bg=self.parent.buttoncolor, activebackground=self.parent.wrapcolor, fg=self.parent.buttonfc).pack(padx=5, side='left')		

		########################################
		cpframe = tk.Frame(self.cmdframe.bodyframe, bg=self.parent.framecolor)
		cpframe.pack()

		tk.Button(cpframe, text="Execute Cycle", command=self.parent.protocol.executeCycle, width=15, relief='flat', bg=self.parent.buttoncolor, activebackground=self.parent.wrapcolor, fg=self.parent.buttonfc).grid(row=0, column=0, padx=5, pady=5)
		tk.Button(cpframe, text="Update Protocol", command=self.parent.protocol.updateProtocol, width=15, relief='flat', bg=self.parent.buttoncolor, activebackground=self.parent.wrapcolor, fg=self.parent.buttonfc).grid(row=1, column=0, padx=5, pady=5)
		tk.Button(cpframe, text="Add Command", command=self.parent.protocol.addCommand, width=15, relief='flat', bg=self.parent.buttoncolor, activebackground=self.parent.wrapcolor, fg=self.parent.buttonfc).grid(row=2, column=0, padx=5, pady=5)
		tk.Button(cpframe, text="Calculate Cycle Times", command=self.parent.protocol.renderCycleTimes, width=15, relief='flat', bg=self.parent.buttoncolor, activebackground=self.parent.wrapcolor, fg=self.parent.buttonfc).grid(row=6, column=0, padx=5, pady=5)
...
#		pumps = ['/dev/ttyUSb0']
		...
		tk.Button(cpframe, text="Reset Pump", command=self.parent.protocol.resetPump, width=15, relief='flat', bg=self.parent.buttoncolor, activebackground=self.parent.wrapcolor, fg=self.parent.buttonfc).grid(row=5, column=0, padx=5, pady=5)
		...
		for i in range(numcycles):
			...
			for cmd in self.protocol:
				if cmd.get('cycle') == i:
					...
					#print(str(tt))
					time += tt
			tk.Label(self.parent.cp.ctframe, text=i, bg=self.parent.darkercolor).grid(row=i+1, column=0, sticky='we')
			tk.Label(self.parent.cp.ctframe, text=str(time)+"sec", bg=self.parent.lightercolor).grid(row=i+1, column=1, sticky='we')
...
class VerticalScrolledFrame(tk.Frame):
	def __init__(self, parent, *args, **kwargs):
		...
		vscrollbar = tk.Scrollbar(self, orient='vertical', relief='flat', troughcolor="#468ef2")
