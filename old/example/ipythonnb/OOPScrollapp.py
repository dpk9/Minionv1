...
class ControlPanel(tk.Frame):
	def __init__(self, parent, root, *args, **kwargs):
		...
		self.primingll = tk.LabelFrame(self, text=" Port Priming ")
		self.primingll.grid(row=0, column=0, padx=5, pady=5, sticky="n")

		self.cmdll = tk.LabelFrame(self, text=" Control Panel ")
...
#			self.lengths.append(length)
		...
		tk.Button(self.cmdll, text="Execute Cycle", command=self.root.update_idletasks, width=12).grid(row=0, column=0, padx=5, pady=5)
...
#		self.command = {}
		...
		self.labelframe = tk.LabelFrame(self, text=" Protocol ", width=1000, height=20)
...
class Test(tk.Frame):
	def __init__(self, parent, *args, **kwargs):
		tk.Frame.__init__(self, parent)
		self.ll = tk.LabelFrame(self, text="Sample LF")
		self.ll.grid(row=0, column=0)
		self.render()

	def render(self):
		for i in range(100):
			tk.Label(self.ll, text="In test obj").grid(row=i, column=0)
...
#		vscroll = tk.Scrollbar(parent)
#		vscroll.pack(side='right')

#		canvas = tk.Canvas(self.parent, yscrollcommand=vscroll.set)
#		canvas.pack()
...
#		self.test.pack()
		...
		self.protocol.pack(side='right', fill='both', expand=True)

#		canvas.create_window(0, 0, anchor="nw", window=self.protocol)

		self.log = Log(self)
		self.log.pack(side='bottom', fill='x', expand=True)

#		tk.Label(self.test, text="In log frame").grid(row=0, column=0)
#		self.log.pack()

		self.cp = ControlPanel(self, self.root)		
...
if __name__ == "__main__":
	...
	class App(tk.Tk):
		def __init__(self, *args, **kwargs):
			...
			self.protocol = Protocol(self.frame.interior)
			self.protocol.pack()
#			for i in range(10):
...
#	for i in range(100):
#		tk.Label(app, text="Text").grid(row=i, column=0)
...
#	app.pack()
	...
	canvas.create_window(0, 0, anchor="nw", window=app)
