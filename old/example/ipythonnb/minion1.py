...
class Safe(object):
        ...
        def __call__(self, *args, **kwargs):
                ...
                except PortsNotSetException, e:
                        tkMessageBox.showerror('Exception', e)
                except Exception, e:
                        tkMessageBox.showerror('Exception', e)

class PortsNotSetException(Exception):
        pass
...
class Protocol(tk.Frame):
	...
	def updateProtocol(self):
                ...
                if self.portsAreSet == False:
                        raise PortsNotSetException, "You must set the ports first!"
	...
	def renderCommand(self, i):
		...
		cmdtype = tk.IntVar()
		tk.Radiobutton(frame, text="One-way", variable=cmdtype, value=0, indicator=0, command=self.commandUpdateInterface, offrelief='flat', bg=color, fg=self.parent.bodyfc, selectcolor=self.parent.buttoncolor, highlightthickness=0).grid(row=0, column=8)
		tk.Radiobutton(frame, text="Cyclic", variable=cmdtype, value=1, indicator=0, command=self.commandUpdateInterface, offrelief='flat', bg=color, fg=self.parent.bodyfc, selectcolor=self.parent.buttoncolor, highlightthickness=0).grid(row=0, column=9)
		...
		sb_volume = tk.Spinbox(frame, textvariable=volume, from_=1, to=1000, width=4, justify='center', buttonbackground=self.parent.headcolor, bg=self.parent.darkentrycolor, bd=1, highlightthickness=0, fg=self.parent.bodyfc)
		...
		minutes = tk.IntVar()
		sb_timemin = tk.Spinbox(frame, textvariable=minutes, from_=0, to=600, command=self.commandUpdateInterface, width=3, justify='center', buttonbackground=self.parent.headcolor, bg=self.parent.darkentrycolor, bd=1, highlightthickness=0, fg=self.parent.bodyfc)
		sb_timemin.grid(row=0, column=15)
		tk.Label(frame, text="min", bg=color, fg=self.parent.bodyfc).grid(row=0, column=16)
		seconds = tk.IntVar()
		sb_timesec = tk.Spinbox(frame, textvariable=seconds, from_=5, to=59, command=self.commandUpdateInterface, width=2, justify='center', buttonbackground=self.parent.headcolor, bg=self.parent.darkentrycolor, bd=1, highlightthickness=0, fg=self.parent.bodyfc)
	...
	def renderCycleTimes(self, *args):
		...
		for i in range(numcycles):
			...
			tk.Label(self.parent.cycleduration.cdframe, text=str(int(time))+" sec", bg=self.parent.framecolor, fg='yellow').grid(row=i+1, column=1, sticky='we')
...
#					speed = 14
				...
				pump.primePort(washport.get(), 1000, speed, 9)
...
if __name__ == "__main__":
	...
	class App(tk.Frame):
		def __init__(self, parent, *args, **kwargs):
			...
			b = tk.Button(self, text='do exception', command=self.cause_exception)
